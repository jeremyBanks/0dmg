...
This gets us 128 iterations, but no more, as mentioned at the end of yesterday's post. We're checking `H`, which we are treating as the less-significant byte out of `HL` because that seems like the little-endian thing to do, but the logic wants us to be waiting for the most-significant bit of the most-significant byte to flip, when we pass below `0x8000 = 0b1000000000000000`. Where am I making a mistake?
